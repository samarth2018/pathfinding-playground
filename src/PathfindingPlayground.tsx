...
export default function PathfindingPlayground() {
  ...
  const pathSet = useMemo(() => new Set(path.slice(0, animate ? animIndex : path.length).map((p) => key(p.r, p.c))), [path, animIndex, animate]);
    ...
    <div className="min-h-screen w-full bg-neutral-50 text-neutral-900 p-4 flex flex-col gap-4">
      <header className="flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex flex-wrap gap-2">
          <button
            className={`px-3 py-1 rounded-2xl border ${mode === "start" ? "bg-black text-white" : "bg-white"}`}
          ...
          <button
            className={`px-3 py-1 rounded-2xl border ${mode === "end" ? "bg-black text-white" : "bg-white"}`}
            onClick={() => setMode("end")}
            title="Place End"
          >
            End
          ...
          <button
            className={`px-3 py-1 rounded-2xl border ${mode === "block" ? "bg-black text-white" : "bg-white"}`}
            onClick={() => setMode("block")}
            title="Draw walls (drag)"
          >
            Block
          </button>
          <button
            className={`px-3 py-1 rounded-2xl border ${mode === "required" ? "bg-black text-white" : "bg-white"}`}
            onClick={() => setMode("required")}
            title="Must-visit waypoint"
          >
            Required
          </button>
          <button
            className={`px-3 py-1 rounded-2xl border ${mode === "erase" ? "bg-black text-white" : "bg-white"}`}
            onClick={() => setMode("erase")}
            title="Erase walls/marks"
          >
            Erase
          </button>
        </div>
      ...
      <section className="flex flex-wrap items-center gap-4">
        ...
        <div className="ml-auto flex gap-2">
          <button className="px-3 py-1 rounded-2xl border bg-white" onClick={exportState}>
            Export
          </button>
          <button className="px-3 py-1 rounded-2xl border bg-white" onClick={importState}>
            Import
          </button>
          <button className="px-3 py-1 rounded-2xl border bg-white" onClick={clearPath}>
            Clear Path
          </button>
          <button className="px-3 py-1 rounded-2xl border bg-white" onClick={resetGrid}>
            Reset Grid
          </button>
          <button className="px-3 py-1 rounded-2xl border bg-black text-white" onClick={run}>
            Run ▶
          </button>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-[auto_260px] gap-4">
          ...
          >
            {grid.flat().map((cell) => (
              <div
                ...
                className={
                  "aspect-square cursor-crosshair rounded-sm border " +
                  (cell.t === "blocked"
        ...
        <aside className="bg-white rounded-2xl p-3 shadow-sm text-sm leading-6">
          ...
          <ol className="list-decimal list-inside space-y-1">
            <li>
              Select <span className="font-medium">Start</span> and click a cell.
            </li>
            <li>
              Select <span className="font-medium">End</span> and click a cell.
            </li>
            <li>
              Draw <span className="font-medium">Block</span> walls (drag to paint).
            </li>
            <li>
              Mark <span className="font-medium">Required</span> waypoints (must-visit).
            </li>
            <li>
              Choose visit order and press <span className="font-medium">Run</span>.
            </li>
          </ol>
          <h3 className="font-semibold mt-3 mb-1">Legend</h3>
          <div className="flex flex-wrap gap-2 items-center">
            <span className="inline-flex items-center gap-2">
              <span className="w-4 h-4 bg-green-500 inline-block rounded-sm border" />
              Start
            </span>
            <span className="inline-flex items-center gap-2">
              <span className="w-4 h-4 bg-red-500 inline-block rounded-sm border" />
              End
            </span>
            <span className="inline-flex items-center gap-2">
              <span className="w-4 h-4 bg-amber-400 inline-block rounded-sm border" />
              Required
            </span>
            <span className="inline-flex items-center gap-2">
              <span className="w-4 h-4 bg-neutral-800 inline-block rounded-sm border" />
              Blocked
            </span>
            <span className="inline-flex items-center gap-2">
              <span className="w-4 h-4 bg-sky-400 inline-block rounded-sm border" />
              Path
            </span>
          </div>
